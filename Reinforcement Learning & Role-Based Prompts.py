...
def reinforcement_learning_activity():
    ...
    prompt = input("Enter aprompt for the AI model(e.g.,''Describe the lion): ").strip()
    if not prompt:
    ...
    print(f"\nImproved AI Respobse : {improved_response}")
    print("\nReflection :")
...
def role_based_prompt_activity():
    ...
    item = input(f"Enter a specific {category} topic (eg, 'photosynthesis' for science)").strip()
