from my_hf import generate_response
import re
...
def complete_answer(question:str,max_rounds:int=2):#Ask for a clean structure(helps avoid unfinished answers)
    base_prompt = (
        "👆Answer clearly in numbered points."
        "🙅‍♀️Do not cut sentences.Finish each point fully.\n\n"
        f"Question:{question}" 
    )

    ans = generate_response(base_prompt, temperature=0.3,max_tokens=1024)
    rounds=0
    while rounds < max_rounds and looks_unfinished(ans):
        cont_prompt = (
