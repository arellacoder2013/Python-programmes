...
ALLOWED, MAX_MB = {".jpg",".jpeg",".png",".bmp",".gif",".webp",".tiff"}, 15

EMOJI = {"person":"🧍‍♀️","car":"🚗","truck":"🚚","bus":"🚌","bicycle":"🚲","motorcycle":"🏍️","dog":"🐶","cat":"🐱",
...
def ask_image():
    print("\n Pick an image (JPG/PNG/WebP/BMP/TIFF <= 15MB) from this folder.")
    while True:
        ...
        if os.path.getsize(p) /(1024*1024)> MAX_MB: print("⚠ Oops! Too big (>15MB)");continue
        try:
            Image.open(p).verify()
        except:
            print("⚠ corrupted image.");continue
        return p
def infer(path, img_bytes, tries=12):
...
def draw(img, dets, thr=0.7):
    ...
    for det in dets[:20]:
        ...
        d.rectangle([(x1,y1),(x2,y2)], outline=color, fill=color)

        txt = f"{lab.upper()} ({s*100:.1f}%)"
        tw,th = d.textlength(txt,font=f), f.size+6

        d.rectangle([(x1,max(0,y1-th)),(x1+tw+8,y1)], fill=color)
        d.text((x1+4,y1-th+3), txt, font=f, fill=(0,255,0))
