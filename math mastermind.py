...
def export_txt(history):
    txt="\n\n".join([f"Q{i}:{h['q']}" for i, h in enumerate(history, 1)])
...
def setup_ui():
    ...
    st.write("Solve any math problem with step-by-step explanations .")
    ...
    if st.session_state.history:
        c2.download_button("💾Export",export_txt(st.session_state.history), "Math Mastermind_solution.txt","text/plain")
    ...
    if not st.session_state.history:
     return st.markdown("### 🧾 Solution History (Latest First)")
